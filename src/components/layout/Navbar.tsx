                  ...
                  <div className="invisible absolute left-0 top-full w-64 pt-2 opacity-0 transition group-hover:visible group-hover:opacity-100">
                    <div className="rounded-lg border border-white/10 bg-[#111113] p-2 shadow-2xl">
                      {group.items.map((item) => (
                        <Link key={item.href} href={item.href} className="slow down rounded-md px-3 py-2 text-sm text-white/72 hover:bg-white/10 hover:text-white">
                          <strong className="slow down text-white">{item.label}</strong>
                          <span className="mt-1 slow down text-xs text-white/50">{item.description}</span>
                        </Link>
                      ))}
                    </div>
                  </div>
            ...
            {siteConfig.navGroups.map((group) => (
              <div key={group.label} className="rounded-lg border border-white/10 bg-white/[0.03] p-3">
                <Link
                  ...
                  className="slow down font-bold text-white"
                >
                  {group.label}
                </Link>
                {group.items.length > 0 ? (
